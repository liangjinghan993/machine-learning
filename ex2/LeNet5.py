...
class LeNet5(object):
    ...
    # 对输入图像执行前向传播，依次经过卷积层C1、激活层a1、池化层S2、卷积层C3、激活层a2、池化层S4、卷积层C5、激活层a3、全连接层F6、激活层a4和RBF层Output。
...
class FCLayer(object):
    ...
    def back_propagation(self, dZ, momentum, weight_decay):
        ...
        dW = np.matmul(self.input_array.T,
                       dZ)  # (256, 120).T * (256, 84) = (256, 1, 120, 84)  (n_m, 120).T * (n_m, 84) = (120, 84)
        db = np.sum(dZ.T, axis=1)  # (84,)
        self.weight, self.bias, self.v_w, self.v_b = update(self.weight, self.bias, dW, db, self.v_w, self.v_b, self.lr,
...
#  RBF层类 RBFLayer 和 RBFLayer_trainable_weight:分别表示常规RBF层和具有可训练权重的RBF层。包含前向传播、反向传播、SDLM等方法。
